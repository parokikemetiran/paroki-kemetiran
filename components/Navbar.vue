<template>
  <nav class="relative w-full p-6 bg-transparent">
    <div class="flex items-center justify-between">
      ...
      <!-- Drawer Menu -->
      <aside class="p-5 transform top-0 left-0 w-3/4 bg-white fixed h-full overflow-auto ease-in-out transition-all duration-300 z-30" :class="isOpen ? 'translate-x-0' : '-translate-x-full'">
        <div class="close">
        ...
        <ul class="font-sans">
            ...
            v-for="item in navigation.data.links"
            :key="$prismic.asText(item.label)"
            class="my-4 w-full inline-block tracking-tight text-slate-800"
            @click="isOpen = false"
          >
            <PrismicLink :field="item.link">
              {{ $prismic.asText(item.label) }}
